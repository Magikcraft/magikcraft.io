...
// TODO: Make this functional to extract strings from a Magikcraft project into a translations.json thing.
...
function processFile(filename, dirJSON) {
    ...
    var translatables = fileContents.match(/localiseMsg\s*\([^\),]*/);
    if(translatables) {
        /* jshint loopfunc: true */
        for(var i = 0; i < translatables.length; i++) {
            if(/^localiseMsg\s*\(\s*['"](.*)['"]$/.test(translatables[i])) { // A string-looking thing
